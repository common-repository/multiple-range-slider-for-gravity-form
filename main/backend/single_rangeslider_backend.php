...
if (class_exists('GF_Field')) {
    class MRSGF_Field_Single_Rangeslider extends GF_Field {

        // Use for type of field
        public $type = 'SingleRangeslider';

        // Use for add a field title
        public function get_form_editor_field_title() { return esc_attr__( 'Single Edge Range slider', 'multiple-range-slider-for-gravity-form-pro' ); }
        ...
        function get_field_input( $form, $value = '', $entry = null ) {
            ...
            $mrsgfgr = "<div class='ginput_container'><div class='single_range_slider ".$mrsgf_class_name."' sliderposition='".$mrsgf_atts['slider_position']."'  step='".$mrsgf_atts['stepvalue']."'  prefix='".$mrsgf_atts['Prefixvalue']."' rangeshow='".$mrsgf_atts['range_show']."'  min='".$mrsgf_atts['min']."'  max='".$mrsgf_atts['max']."' ><input  name='input_".$mrsgfid."'  id='". $mrsgf_form_id."' value='".$mrsgf_valueee."' type='".$mrsgf_atts['type']."'/></div></div>";

            return $mrsgfgr;
        }
    }
    GF_Fields::register(new MRSGF_Field_Single_Rangeslider() );
}


// use for gravity form field standard settings
add_action( 'gform_field_standard_settings', 'MRSGF_add_single_custom_field' , 10, 2 );
function MRSGF_add_single_custom_field( $position, $form_id ){      
    ...
    if ($position == 25) {
        ...
        <li class="slider_range field_setting">
            <label for="slider_range" class="section_label">
                   <?php esc_html_e('Range', 'multiple-range-slider-for-gravity-form-pro'); ?>
            </label>
            <?php esc_html_e('min', 'multiple-range-slider-for-gravity-form-pro'); ?>
            <input type="number" id="slider_range_min"  name="min" min="1"  onchange="SetFieldProperty('min', this.value);" /></br>
            <?php esc_html_e('max', 'multiple-range-slider-for-gravity-form-pro'); ?>
        ...
        <li class="stepvalue field_setting">
            <label for="stepvalue" class="section_label">
                   <?php esc_html_e('Step :', 'multiple-range-slider-for-gravity-form-pro'); ?>
            </label>
        ...
        <li class="range_show field_setting">
            ...
            <input type="radio" class="gf_rs_radio" name="rangeshow" value="enable"   onchange="SetFieldProperty('range_show', this.value);" disabled=""/>
            <?php esc_html_e('Disable', 'multiple-range-slider-for-gravity-form-pro'); ?>
            <input type="radio" class="gf_rs_radio" name="rangeshow" value="disable"  onchange="SetFieldProperty('range_show', this.value);" disabled=""/>
            <label class="mrsfgf_comman_link"><?php echo __('Only available in ','multiple-range-slider-for-gravity-form');?> <a href="https://www.plugin999.com/plugin/multiple-range-slider-for-gravity-form/" target="_blank">Pro Version</a></label>
        </li>
        <?php 
    }      
...
add_action('gform_editor_js', 'MRSGF_editor_single_script', 11, 2);
function MRSGF_editor_single_script() {
    ?>
...
add_action( 'gform_editor_js_set_default_values', 'MRSGF_set_default_single_values' );
function MRSGF_set_default_single_values(){
    ?>
    case "SingleRangeslider" :
        field.label = 'Single Edge Range Slider';
        field.min = 1;
        field.max = 23;
        field.stepvalue = 4;
        field.Prefixvalue = '%';
